...
function plot_grid_world(mdp::MDP,
    ...
    if show_rewards
        for s in filter(s->0 != 0, states(mdp))#filter(s->reward(mdp, s) != 0, states(mdp))
            r = 0#reward(mdp, s)
    ...
    if !isnothing(outline_state)
        terminal_states = filter(s->0 != 0, states(mdp))#filter(s->reward(mdp, s) != 0, states(mdp))
...
manifest_format = "2.0"
project_hash = "60a0e6b326de314ab09fd55cd55cbe0235a09617"
