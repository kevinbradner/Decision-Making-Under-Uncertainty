...
function T(s::State, a::Action, debug=missing)
	...
	sigma= pi/16
	mvt_gaussian = Normal(wfv_angle, sigma)
...
action(vi_policy, State(4,3))

# ╔═╡ b5834c6a-687b-4b74-869c-6f01fa066fdb
value(vi_policy, State(4,3))
...
function plot_grid_world(mdp::MDP,
	...
	if show_wind
		for xq=1:xmax
	 		yq= [1,2,3,4,5,6,7]
   			fig=quiver!(xq*[1,1,1,1,1,1,1], yq, quiver=(0.03*uq[xq,:],0.03*vq[xq,:]), 			c=:red,linewidth=2)
		end
	end	
...
manifest_format = "2.0"
project_hash = "9c6d29af62bcd6b60fe8229a82947a161ff75039"
